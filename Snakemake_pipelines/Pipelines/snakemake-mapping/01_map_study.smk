...
configfile: "./map_config.yaml" #"/storage/home/hcoda1/6/rridley3/scratch/subset_2/config.yaml"
...
temp_scratch_dir= str(config['study_scratch']).strip() + '/smk_temp'
...
rule map_p1_set_genomes:
    ...
    --bam-file-cache-directory {output.bam_files} -1 {input.read1} -2 {input.read2} | tee -a {log}
    """
...
rule get_tads_gene:
    ...
    output:
        ...
        out_cov_tpm= f"{out_name}/03_gene_analysis/00_coverage/01_set_genes_tpm-{{group}}.tsv",
    ...
    TMPDIR={params.tmpdir} coverm contig -m tpm \
    --output-file {output.out_cov_tpm}   \
     --min-read-percent-identity {params.min_id} \
    --min-read-aligned-percent {params.min_aln} -t {threads} \
    -b {input.bam_files}/* | tee -a {log}
...
rule get_tads_unbin:
    ...
    output:
        ...
        out_cov_tpm=f"{out_name}/03_gene_analysis/00_coverage/02_unbin_genes_tpm-{{group}}.tsv",
