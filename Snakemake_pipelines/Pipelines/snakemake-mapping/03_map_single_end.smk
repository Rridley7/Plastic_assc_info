...
configfile: "./se_map_config.yaml" #"/storage/home/hcoda1/6/rridley3/scratch/subset_2/config.yaml"
localrules: all, get_sra_output

group_range = config['group_range']
group_list = ['M' + str(x).zfill(2) for x in list(range(int(group_range[0]),int(group_range[1])+1))]

out_name = config['out_dir']
out_name = os.getcwd() + '/' + out_name

data_file = config["study-file"]
df = pd.read_parquet(data_file).set_index("SampleID", drop=False).astype('str')
working_df = df.query("map_group in @group_list")
read_type_df = working_df[['map_group','read_type']].drop_duplicates(ignore_index=True).set_index('map_group')
...
temp_scratch_dir= str(config['se_scratch']).strip() + '/smk_temp'
...
#     """
def get_reads(map_group,read):
    return df.query(f'map_group == @map_group')[read].to_list()
map_types = dict(single='bwa-mem2',long='minimap2-pb')
def get_ref(group,map_type,read_set):
    # Map type is genomes (bins) or genes
    # Read set is 01_set or 02_unbin
    mapper = map_types[read_type_df.loc[group,'read_type']]
    if mapper == 'bwa-mem2':
        return config[map_type][read_set]
    elif mapper == 'minimap2-pb':
        return config[f'fa-{map_type}'][read_set]

rule map_p1_set_genomes:
    input:
        read1 = lambda wildcards: ancient(get_reads(wildcards.group,'Read1')),
        #read2 = lambda wildcards: ancient(get_reads(wildcards.group,'Read2')),
    output:
    ...
    params:
        ...
        ref_file= lambda wildcards: get_ref(wildcards.group,'bins','01_set'),
        read_dir = lambda wildcards: f"{out_name}/01_trimmed/{wildcards.group}",
        mapper = lambda wildcards: map_types[read_type_df.loc[wildcards.group,'read_type']],
        tmpdir = lambda wildcards: f"{temp_scratch_dir}/coverm/{wildcards.group}/map"
    shell:"""
    mkdir -p {params.tmpdir} {output.bam_files}


    TMPDIR={params.tmpdir} coverm genome -m trimmed_mean -r {params.ref_file} \
    --output-file {output.out_cov} -p {params.mapper} --trim-max {params.trim_max} --trim-min \
    {params.trim_min} --separator '~' --min-read-percent-identity {params.min_id} \
    --min-read-aligned-percent {params.min_aln} -t {threads} \
    --bam-file-cache-directory {output.bam_files} --single {input.read1}  | tee -a {log}
    """

def samtools_out(group):
    if group in ['M34','M35']:
        return 'fasta'
    else:
        return 'fastq'
...
rule filter_genomes:
    ...
    params:
        ...
        samtools_type = lambda wildcards: samtools_out(wildcards.group),
    ...
    shell: """        
        ...
        samtools collate -O -@ {threads} $file $TEMPFILE | samtools {params.samtools_type} -0 {output.FQ_DIR}/${{OUT_FILE}}.gz  \
        -s /dev/null -@ {threads} -
        done
        #rm {input.BAM_DIR}/*
        #rm {output.UMP_DIR}/*
...
rule map_p2_unbin:
    ...
    params:
        ...
        trim_max=config['coverm']['trim_max'],
        mapper= lambda wildcards: map_types[read_type_df.loc[wildcards.group,'read_type']],
        ref_file= lambda wildcards: get_ref(wildcards.group,'bins','02_unbin'),
        add_params=config['coverm']['add_params'],
        tmpdir = lambda wildcards: f"{temp_scratch_dir}/coverm/{wildcards.group}/map"
    shell: """
    mkdir -p {params.tmpdir}
    
    TMPDIR={params.tmpdir} coverm genome -m trimmed_mean -r {params.ref_file}  --single-genome \
    --output-file {output.out_cov} -p {params.mapper} --trim-max {params.trim_max} --trim-min \
    {params.trim_min}  --min-read-percent-identity {params.min_id} --min-read-aligned-percent {params.min_aln} -t {threads} \
    --bam-file-cache-directory {output.bam_files} --single {input.in_dir}/*.gz | tee -a {log}
...
rule map_genes:
    ...
    params:
        mapper= lambda wildcards: map_types[read_type_df.loc[wildcards.group,'read_type']],
        ref_file= lambda wildcards: get_ref(wildcards.group,'genes','01_set'),
        min_id=config['coverm']['min_id'],
        min_aln=config['coverm']['min_aln'],
        trim_min=config['coverm']['trim_min'],
        trim_max=config['coverm']['trim_max'],
    ...
    if [[ '{wildcards.group}' == 'M38' ]] 
    then
    conda activate coverm
    fi

    TMPDIR={params.tmpdir} coverm contig -m trimmed_mean -r {params.ref_file} \
    --output-file {output.out_cov} -p {params.mapper} --trim-max {params.trim_max} --trim-min \
    {params.trim_min}  --min-read-percent-identity {params.min_id} --min-read-aligned-percent {params.min_aln} -t {threads} \
    --bam-file-cache-directory {output.bam_files} --single {input.read1}  | tee -a {log}
    """
...
rule filter_genes:
    ...
    shell: """
        ...
        OUT_FILE=`echo $file | cut -f2-3 -d. | sed 's|-t|-ump|g'`
        TEMPFILE={params.tmpdir}/${{OUT_FILE}}_TEMPBAM
        samtools collate -O -@ {threads} $file $TEMPFILE | samtools {params.samtools_type} -0 {output.FQ_DIR}/${{OUT_FILE}}.gz  -s /dev/null -@ {threads} -
        done
...
rule map_unbin_genes:
    ...
    resources:
        ...
        mem_mb=get_mem2_mb,
    params:
        ref_file= lambda wildcards: get_ref(wildcards.group,'genes','02_unbin'),
        mapper= lambda wildcards: map_types[read_type_df.loc[wildcards.group,'read_type']],
        min_id=config['coverm']['min_id'],
        min_aln=config['coverm']['min_aln'],
        trim_min=config['coverm']['trim_min'],
    ...
    --output-file {output.out_cov} -p {params.mapper} --trim-max {params.trim_max} --trim-min \
    {params.trim_min}  --min-read-percent-identity {params.min_id} --min-read-aligned-percent {params.min_aln} -t {threads} \
    --bam-file-cache-directory {output.bam_files} --single {input.read_dir}/*.gz | tee -a {log}
